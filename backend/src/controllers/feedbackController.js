...
export const respondToFeedback = async (req, res) => {
  try {
    ...
    // Verify feedback belongs to organizer's event
    if (feedback.organizerId.toString() !== organizerId) {
      return res.status(403).json({
        success: false,
        message: "Unauthorized",
      });
    ...
    feedback.organizerResponse = response;
    feedback.organizerResponded = true;
...
// Publish feedback (make it public)
...
// Hide feedback (remove from public view)
...
export const getPublicFeedbacksForEvent = async (req, res) => {
  try {
    const { eventId } = req.params;
...
export const getAverageRatingForEvent = async (req, res) => {
  try {
    ...
    const stats = result[0] || { averageRating: 0, totalFeedbacks: 0 };
